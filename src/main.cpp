...
#if defined(NDEBUG)
# error "Atocs cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4aae1fea286216147d1ea1ef4783cd3fcb378b712576029423c0e057e6adf223");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Atocs: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Atocs Signed Message:\n";
...
{
    // Atocs: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Atocs: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Atocs: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Atocs: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Atocs: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Atocs: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3bed564594cda269b35ff8fdc814139ad1307c3549328c92199b2bd71507d7e2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Is 2023 and I am still alive";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0410e467085531ad0c3390964f70de60405179761e7016d3d4742fb5e066047dd1b1a19f0b0ce75196232b21508c156f1087a428e99d0716ac29716f90f730d2f2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1673824953;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086306423;

        if (fTestNet)
        {
            block.nTime    = 1673824938;
            block.nNonce   = 387322633;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x2197657e0fc1abc13cdbe2d458c88f365e5880d6d2bb7b859e5ec0cb032a5e45"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc5, 0xb4, 0xd7 }; // Atocs: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AtocsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AtocsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AtocsMiner(CWallet *pwallet)
{
    printf("AtocsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("atocs-miner");
    ...
    try { loop {
        ...
        printf("Running AtocsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AtocsMiner, pwallet));
}
